...
function generateDescriptions(){
	...
	urls.push("Water & Wastewater");
	urls.push("Transportation & Roadway ");
	urls.push("Health");
	urls.push("Baltimore City Liquor");
	urls.push("Roadside Maintenance");
	urls.push("Financial Complaints");
}

function setLayers(string) {
switch(string){
case "":
case "Sanitation & Waste": var SW_url = base_url +"&$select=codedescription,location,address&$where=starts_with(codedescription,'SW')";
currentIcon = new L.Icon.Default();getMarkers(SW_url, SW,currentIcon);
case "Forestry Services":var Forest_url = base_url + "&$select=codedescription,location,address&$where=starts_with(codedescription,'Forestry')";
currentIcon = greenIcon;getMarkers(Forest_url, Forestry,currentIcon);
case "Housing & Community Development":var HCD_url = base_url +"&$select=codedescription,location,address&$where=starts_with(codedescription,'HCD')";
currentIcon = pinkIcon;getMarkers(HCD_url, HCD,currentIcon);
case "Transportation & Roadway Maintenance":var TRM_url = base_url +"&$select=codedescription,location,address&$where=starts_with(codedescription,'TRM')";
currentIcon = blackIcon;getMarkers(TRM_url, TRM,currentIcon);
case "Transportation & Roadway Service": var TRS_url = base_url +"&$select=codedescription,location,address&$where=starts_with(codedescription,'TRS')";
currentIcon = fuschaIcon;getMarkers(TRS_url, TRS,currentIcon);
case "Water & Wastewater":var WW_url = base_url +"&$select=codedescription,location,address&$where=starts_with(codedescription,'WW')";
currentIcon = tealIcon;getMarkers(WW_url, WW,currentIcon);
case "Transportation & Roadway ":var TRT_url = base_url +"&$select=codedescription,location,address&$where=starts_with(codedescription,'TRT')";
currentIcon = yellowIcon;getMarkers(TRT_url, TRT,currentIcon);
case "Health":var HLTH_url = base_url +"&$select=codedescription,location,address&$where=starts_with(codedescription,'HLTH')";
